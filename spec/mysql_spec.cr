...
describe MySQL do
  ...
  describe "#connect" do
    it "fails with MySQL::ConnectionError when unable to connect" do
      expect_raises(MySQL::Errors::Connection, /Can't connect to MySQL server/) do
  ...
  describe "#query" do
    ...
    it "raises NotConnectedError when client is not connected" do
      expect_raises(MySQL::Errors::NotConnected) do
  ...
  describe "#transaction" do
    ...
    context "when using transactions" do
      ...
      it "raises UnableToRollbackTransaction when it is unable to roll it back" do
        ...
        expect_raises(MySQL::Errors::UnableToRollbackTransaction, /Errors::NotConnected/) do
          conn.transaction do
            conn.query(%{INSERT INTO user values(1, 'john@example.org', 'John')})
            conn.query(%{SELECT COUNT(id) FROM user}).should eq([[1]])
            conn.close
